...
from EasyLM.models.olmo.olmo_model import (
    OLMoConfig, FlaxOLMoForCausalLM, FlaxOLMoForSequenceClassification, FlaxOLMoForTokenRegression, OlmoTokenizer
)
...
FLAGS, FLAGS_DEF = mlxu.define_flags_with_default(
    ...
    dtype='bf16',
    load_olmo_config_policy='',
    load_olmo_config_reward='',
    update_olmo_config_policy='',
    update_olmo_config_reward='',
...
def main(argv):
    ...
    tokenizer = OlmoTokenizer.from_pretrained(FLAGS.tokenizer, padding_side='left', truncation_side='left')
    ...
    if FLAGS.load_olmo_config_policy != '':
        olmo_config_policy = OLMoConfig.load_config(FLAGS.load_olmo_config_policy)
    else:
        olmo_config_policy = OLMoConfig(**FLAGS.olmo)
    if FLAGS.update_olmo_config_policy != '':
        olmo_config_policy.update(dict(eval(FLAGS.update_olmo_config_policy)))
    olmo_config_policy.update(dict(
        bos_token_id=wrapped_dataset.tokenizer.bos_token_id,
        eos_token_id=wrapped_dataset.tokenizer.eos_token_id,
    ))
    if olmo_config_policy.vocab_size < wrapped_dataset.vocab_size:
        olmo_config_policy.update(dict(vocab_size=wrapped_dataset.vocab_size))

    if FLAGS.load_olmo_config_reward != '':
        olmo_config_reward = OLMoConfig.load_config(FLAGS.load_olmo_config_reward)
    else:
        olmo_config_reward = OLMoConfig(**FLAGS.olmo)
    if FLAGS.update_olmo_config_reward != '':
        olmo_config_reward.update(dict(eval(FLAGS.update_olmo_config_reward)))
    olmo_config_reward.update(dict(
        bos_token_id=wrapped_dataset.tokenizer.bos_token_id,
        eos_token_id=wrapped_dataset.tokenizer.eos_token_id,
    ))
    if olmo_config_reward.vocab_size < wrapped_dataset.vocab_size:
        olmo_config_reward.update(dict(vocab_size=wrapped_dataset.vocab_size))

    policy_model = FlaxOLMoForCausalLM(olmo_config_policy, dtype=get_float_dtype_by_name(FLAGS.dtype), _do_init=False)
    value_model = FlaxOLMoForTokenRegression(olmo_config_reward, dtype=get_float_dtype_by_name(FLAGS.dtype), _do_init=False)
    reference_model = FlaxOLMoForCausalLM(olmo_config_policy, dtype=get_float_dtype_by_name(FLAGS.dtype), _do_init=False)
    reward_model = FlaxOLMoForSequenceClassification(olmo_config_reward, dtype=get_float_dtype_by_name(FLAGS.dtype), _do_init=False)
    ...
    def init_fn_policy(rng):
        ...
        params = policy_model.module.init(
            ...
            rngs=rng_generator(olmo_config_policy.rng_keys()),
        )
    ...
    train_state_partition_policy = match_partition_rules(OLMoConfig.get_partition_rules(), train_state_shapes_policy)
    ...
    def init_fn_reward(rng):
        ...
        params = reward_model.module.init(
            ...
            position_ids=jnp.zeros((4, seq_length), dtype=jnp.int32),
            attention_mask=jnp.ones((4, seq_length), dtype=jnp.int32),
            rngs=rng_generator(olmo_config_reward.rng_keys()),
        )
    ...
    train_state_partition_reward = match_partition_rules(OLMoConfig.get_partition_rules(), train_state_shapes_reward)
    ...
    def save_checkpoint(policy_train_state, value_train_state, step, milestone=False):
        metadata = dict(
            ...
            olmo_config=olmo_config_policy.to_dict(),
        )
