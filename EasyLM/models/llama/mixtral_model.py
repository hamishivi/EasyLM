'''
Mixtral code, adapted from Llama code.
'''
import os
...
class MixtralConfig(PretrainedConfig):
    ...
    def get_partition_rules():
        """ Parition rules for mixtral. Note that these rules are orderd, so that
    ...
    def get_tokenizer(cls, config, padding_side='left', truncation_side='right'):
        ...
        # mixtral tokenizer is just llama tokenizer
...
# these are all the same as in Llama.
from llama_model import RMSNorm, precompute_freqs_cis, apply_rotary_emb


# same as llama Attention
# TODO: support sliding window attention.
# it doesnt seem like this is configured in the huggingface code?
...
# regular MLP, same as llama for here.
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def _compute_expert_hidden_states(self, hidden_states)
    ...
    def __call__(self, hidden_states: jnp.ndarray, deterministic: bool = True) -> jnp.ndarray:
        ...
        # routing stuff should all be in high precision
        routing_weights = jax.nn.softmax(router_logits.astype(jnp.float32), axis=1)
        # n.b.: jax topk is along last axis
        routing_weights, selected_experts = jax.lax.topk(routing_weights, self.top_k)
        ...
        expert_mask = jax.nn.one_hot(selected_experts, num_classes=self.num_experts).permute(2, 1, 0)

        # in pytorch, we loop. In jax, we can scan instead
        lax.scan(
            self._compute_expert_hidden_states,
            (hidden_states, routing_weights, expert_mask),
            final_hidden_states,
        )
        )
